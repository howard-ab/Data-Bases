...
-- 12. Функция ранжирования get_competition_ranking(olympiad_id integer):
create or replace function get_competition_ranking(olympiad_id integer)
returns table(place integer, swimmer_name varchar) as $$
begin
    return query
    select row_number() over(order by distance_time) as place, swimmer_name from competition where olympiad_id = olympiad_id;
end;
$$ language plpgsql;



-- 13.Реализации функции смешения get_coach_salary_by_shift на языке PL/pgSQL:
create or replace function get_coach_salary_by_shift(coach_id integer, shift integer)
returns numeric as $$
declare
    salary numeric;
begin
    select c.salary * power(1.05, shift) into salary
    from coach c
